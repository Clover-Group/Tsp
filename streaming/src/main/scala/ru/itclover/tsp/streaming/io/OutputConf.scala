...
import fs2.Pipe
import fs2.kafka.{Acks, KafkaProducer, ProducerRecord, ProducerSettings, Serializer}
...
case class JDBCOutputConf(
  ...
  def createTableQuery: ConnectionIO[Int] =
    ...
    if driverName.toLowerCase().contains("clickhouse") then fragment ++= fr"engine = Log()"
    fragment.update.run
  ...
  def fuseMapGeneric[F[_], A, B, C](
    ...
    def mkStream(c: Connection): fs2.Stream[F, C] =
      ...
      def evalS(f: ConnectionIO[?]): fs2.Stream[F, Nothing] =
