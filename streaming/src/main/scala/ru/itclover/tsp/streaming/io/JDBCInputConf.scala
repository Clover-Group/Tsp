...
/** Source for anything that support JDBC connection
  ...
  * @param datetimeField
  *   name of datetime field, could be timestamp and regular time (will be parsed by JodaTime)
  * @param eventsMaxGapMs
  *   maximum gap by which source data will be split, i.e. result incidents will be split by these gaps
  * @param defaultEventsGapMs
  *   "typical" gap between events, used to unite nearby incidents in one (sessionization)
  * @param partitionFields
  *   fields by which data will be split and paralleled physically
  * @param userName
  *   for JDBC auth
  * @param password
  *   for JDBC auth
  * @param parallelism
  *   of source task (not recommended to chagne)
  * @param numParallelSources
  *   number of absolutely separate sources to create. Patterns also will be separated by equal (as much as possible)
  *   buckets by the max window in pattern (TBD by sum window size)
  * @param patternsParallelism
  *   number of parallel branch nodes splitted after sink stage (node). Patterns also separated by approx. equal buckets
  *   by the max window in pattern (TBD by sum window size)
  */
