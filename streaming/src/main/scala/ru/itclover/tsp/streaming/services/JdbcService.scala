...
object JdbcService:
  ...
  def fetchFieldsTypesInfo(driverName: String, jdbcUrl: String, query: String): Try[Seq[(String, Class[?])]] =
    val classTry: Try[Class[?]] = Try(Class.forName(driverName))
    ...
    yield (1 to metaData.getColumnCount).map { (i: Int) =>
  ...
  def fetchAvailableKeys(driverName: String, jdbcUrl: String, query: String, keyColumn: String): Try[Set[String]] =
    ...
    yield new Iterator[String] {
      def hasNext = resultSet.next()
      def next() = resultSet.getString(1)
    }.to(LazyList).map(String(_)).toSet
