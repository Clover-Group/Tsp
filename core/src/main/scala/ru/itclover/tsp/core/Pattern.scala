...
object Pat:
  ...
  def unapply[E, T](arg: Pat[E, T]): Option[Pattern[E, ?, T]] = arg match
    case x: Pattern[?, _, ?] => Some(x.asInstanceOf[Pattern[E, ?, T]])
    case _ =>
...
/** Main trait for all patterns, basically just a function from state and events chunk to new a state.
  ...
  *   underlying data
  * @tparam T
  *   Type of the results in the S
  * @tparam S
  *   Holds State for the next step AND results (wrong named `queue`)
  */
trait Pattern[Event, S, T] extends Pat[Event, T] with Serializable:
  ...
  /** @param oldState
    ...
    * @tparam F
    *   Container for state (some simple monad mostly)
    * @tparam Cont
    *   Container for yet another chunk of Events
    * @return
    *   new state wrapped in F[_]
    */
