...
object PatternMemoryCheck {
  ...
  private def runAndReturnFinalState[A, S](pattern: Pattern[A, S, ?], events: Seq[A], groupSize: Int) = {
  ...
  def finalStateSize[A, S <: AnyRef](pattern: Pattern[A, S, ?], events: Seq[A], groupSize: Int): Long = {
    val finalState = runAndReturnFinalState(pattern, events, groupSize)
    val layout = GraphLayout.parseInstance(finalState)
    layout.totalSize()
  }

  def finalStateSize[A, S <: AnyRef](pattern: Pattern[A, S, ?], event: A, amount: Int, groupSize: Int): Long = {
